...
def integration_test_fixer_agent(state: AgentState) -> AgentState:
    failures = state.get("integration_test_failures", []) or []
    test_map = state.get("integration_test_map", {}) or {}
    error_log = state.get("latest_error_log", "") or ""

    if not failures:
        return {"integration_retry_count": state.get("integration_retry_count", 0) + 1}

    llm = get_llama_model()
    max_fixes = int(state.get("integration_max_fixes", 50))

    for test_file in failures[:max_fixes]:
        test_content = read_file(test_file)
        source_file = test_map.get(test_file, "")
        source_content = read_file(source_file) if source_file else ""
        relevant_log = _extract_relevant_log(error_log, test_file)

        prompt = f"""
        The following integration test is failing. Fix the test file ONLY (do not change source files).
